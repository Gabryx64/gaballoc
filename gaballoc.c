...
slab_t* NOOPT small_slab_create(size_t size, size_t align, slab_t* prev,
				slab_t* next) {
    ...
    ret->maxrefs =
	(GABALLOC_PAGE_SIZE - sizeof(slab_t)) / (sizeof(bufctl_t) + align);
...
void* NOOPT small_slab_alloc(slab_t* slab, void* source, size_t size,
			     size_t align, slab_t* first_slab,
			     slab_t** free_slab) {
	...
	return small_slab_alloc(slab->next, source, size, align, first_slab,
				free_slab);
    }
...
slab_t* NOOPT large_slab_create(size_t size, size_t align, slab_t* prev,
				slab_t* next) {
    void* mem = GABALLOC_ALLOC(1);
    slab_t* ret = mem + GABALLOC_PAGE_SIZE - sizeof(slab_t);
    ret->prev = prev ? prev : ret;
	...
	buf->next = (bufctl_t*)mem;
	((bufctl_t*)mem)->prev = buf;
	buf->mem = GABALLOC_ALLOC(DIV_RNDUP(size + sizeof(bufctl_t*),
					    GABALLOC_PAGE_SIZE)) +
		   sizeof(bufctl_t*);
...
void NOOPT large_slab_destroy(size_t size, slab_t* slab) {
    ...
    bufctl_t* curr = first;
    do
	GABALLOC_FREE(curr->mem - sizeof(bufctl_t*),
		      DIV_RNDUP(size + sizeof(bufctl_t*), GABALLOC_PAGE_SIZE));
    while (curr != first);

    GABALLOC_FREE(slab->firstbuf, 1);
}

void* NOOPT large_slab_alloc(slab_t* slab, void* source, size_t size,
			     size_t align, slab_t* first_slab,
			     slab_t** free_slab) {
    if (slab->refcount >= slab->maxrefs) {
	if (slab->next == first_slab) {
	...
	return large_slab_alloc(slab->next, source, size, align, first_slab,
				free_slab);
    }
...
cache_t* cache_create(size_t size, size_t align,
		      void (*constructor)(void* ptr, size_t size),
		      void (*destructor)(void* ptr, size_t size)) {
    GABALLOC_LOCK();

    cache_t* ret =
	GABALLOC_ALLOC(DIV_RNDUP(sizeof(cache_t) + size, GABALLOC_PAGE_SIZE));
    ret->size = size;
    ret->align = align < size ? size : align;
    ret->large = align >= (GABALLOC_PAGE_SIZE / 8);
    ret->slabs = (ret->large ? large_slab_create
			     : small_slab_create)(size, align, NULL, NULL);
...
void cache_destroy(cache_t* cache) {
	...
	slab_t* slab = curr;
	curr = curr->next;
	cache->large ? large_slab_destroy(cache->size, slab)
		     : small_slab_destroy(slab);
    } while (curr != first);
...
void* cache_alloc(cache_t* cache) {
    ...
    void* ret = (cache->large ? large_slab_alloc : small_slab_alloc)(
	cache->slabs, cache->source, cache->size, cache->align, cache->slabs,
	&cache->slabs);
